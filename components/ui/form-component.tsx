...
import { TelescopeIcon, X, Check, ChevronsUpDown, Globe, Cpu } from 'lucide-react';
    ...
    return (
      <>
        <Popover open={open} onOpenChange={setOpen}>
          ...
          <PopoverContent
            className="w-[90vw] sm:w-[16em] max-w-[16em] p-0 font-sans rounded-xl bg-white/30 backdrop-blur-lg border border-white/30 shadow-lg mt-0"
            align="start"
            side="bottom"
            sideOffset={20}
              ...
              <CommandList className="max-h-[15em]">
                {Object.entries(groupedModels).map(([category, categoryModels], categoryIndex) => {
                  if (category.toLowerCase() === 'experimental') {
                    // Comment out/hide the experimental models section
                    return null;
                  }
                  const enabledModels = categoryModels.filter(model => {
                    const requiresAuth = requiresAuthentication(model.value) && !user;
                    const requiresPro = requiresProSubscription(model.value) && !isProUser;
                    return !requiresAuth && !requiresPro;
                  });
                  if (enabledModels.length === 0) {
                    // No enabled models in this group, so skip rendering the label and group
                    return null;
                  }
                  return (
                ...
                <CommandGroup key="coming-soon">
                  <div className="my-1 border-t border-border" />
                  <div className="px-2 py-1 text-[10px] font-medium text-muted-foreground">Coming Soon</div>
        ...
        <Popover open={open} onOpenChange={setOpen}>
          <Tooltip>
            ...
            <TooltipContent side="bottom">
              <p>Choose search mode</p>
            </TooltipContent>
          </Tooltip>
          <PopoverContent
            className="w-[90vw] sm:w-[14em] max-w-[14em] p-0 font-sans rounded-xl bg-white/20 backdrop-blur-md border border-white/30 shadow-lg mt-0"
            align="start"
            side="bottom"
            sideOffset={20}
            avoidCollisions={true}
              ...
              <CommandList className="max-h-[240px]">
                {Object.entries(
                  visibleGroups.reduce((acc, group) => {
                    ...
                    return acc;
                  }, {} as Record<string, typeof visibleGroups>)
                ).map(([category, groups], idx, arr) => (
                  <CommandGroup key={category}>
                    {category === 'Crypto & Finance' && <div className="my-1 border-t border-border" />}
              ...
              >
                ...
                <div className={cn('flex items-center flex-shrink-0 gap-2')}>
                      ...
                      <TooltipTrigger asChild>
                        ...
                        >
                          <span className="block">
                            <img src="/toolicon.svg" alt="LCX Exchange" className="h-5 w-5" />
                          </span>
                        </button>
                      </TooltipTrigger>
                      <TooltipContent
                  ...
                  {isProcessing ? (
                    ...
                    // Voice Recording Button commented out - removed for now
                    null
                  ) : (
