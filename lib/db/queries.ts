...
export async function getGuestUsageByIP({ ipAddress }: { ipAddress: string }) {
  try {
    const now = new Date();
    // Start of current day
    const startOfDay = new Date(now.getFullYear(), now.getMonth(), now.getDate());
    ...
    const [usage] = await db
      ...
      .from(guestUsage)
      .where(
        and(eq(guestUsage.ipAddress, ipAddress), gte(guestUsage.date, startOfDay), lt(guestUsage.date, startOfNextDay)),
      )
      .limit(1);

    return usage;
  } catch (error) {
    throw new ChatSDKError('bad_request:database', 'Failed to get guest usage');
  }
}

export async function incrementGuestUsage({ ipAddress, userAgent }: { ipAddress: string; userAgent?: string }) {
  try {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    ...
    // Clean up previous day entries for this IP
    await db.delete(guestUsage).where(and(eq(guestUsage.ipAddress, ipAddress), lt(guestUsage.date, today)));

    const existingUsage = await getGuestUsageByIP({ ipAddress });
    ...
    if (existingUsage) {
      const [updatedUsage] = await db
        ...
        .set({
          searchCount: existingUsage.searchCount + 1,
          userAgent: userAgent || existingUsage.userAgent,
          updatedAt: new Date(),
        })
        .where(eq(guestUsage.id, existingUsage.id))
        .returning();
      return updatedUsage;
    } else {
      const [newUsage] = await db
    ...
    throw new ChatSDKError('bad_request:database', 'Failed to increment guest usage');
  }
}

export async function getGuestUsageCount({ ipAddress }: { ipAddress: string }): Promise<number> {
  try {
    const usage = await getGuestUsageByIP({ ipAddress });
    return usage?.searchCount || 0;
  } catch (error) {
