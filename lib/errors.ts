...
export function getMessageByErrorCode(errorCode: ErrorCode): string {
  ...
  switch (errorCode) {
    ...
    case 'rate_limit:guest':
      return 'You have reached your daily limit for guest searches. Please wait for 24 hours.';
...
export function getErrorIcon(error: ChatSDKError): 'warning' | 'error' | 'upgrade' | 'auth' | 'limit' {
  if (isSignInRequired(error)) return 'auth';
  if (isProRequired(error)) return 'upgrade';
  if (isRateLimited(error)) return 'limit';
