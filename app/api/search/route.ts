...
const GUEST_DAILY_LIMIT = SEARCH_LIMITS.DAILY_SEARCH_LIMIT; // Guest users get the configured number of searches per day
...
export async function POST(req: Request) {
  ...
  if (user) {
    ...
    // Get guest IP for usage tracking
    const guestIP = req.headers.get('x-forwarded-for') || 
                   req.headers.get('x-real-ip') || 
                   'unknown';
    const userAgent = req.headers.get('user-agent') || null;
    
    // Check current guest usage
    const currentUsage = await getGuestUsageCount({ ipAddress: guestIP });
    
    if (currentUsage >= GUEST_DAILY_LIMIT) {
      return new ChatSDKError('rate_limit:guest', `Guest daily limit of ${GUEST_DAILY_LIMIT} searches reached. Please wait for 24 hours.`).toResponse();
    }

    // Increment guest usage
    await incrementGuestUsage({ ipAddress: guestIP, userAgent: userAgent || undefined });
    const newUsage = currentUsage + 1;

    console.log(`👤 Guest usage: ${newUsage}/${GUEST_DAILY_LIMIT} (IP: ${guestIP})`);
    ...
    execute: async (dataStream) => {
      ...
      const result = streamText({
        ...
        tools: {
          ...
          // LCX Exchange Tools
          lcx_kline: lcxKlineTool,
          lcx_order_book: lcxOrderBookTool,
          lcx_ticker: lcxTickerTool,
          lcx_pairs: lcxPairsTool,
          lcx_pair: lcxPairTool,
          lcx_tickers: lcxTickersTool,
          lcx_trades: lcxTradesTool,
          lcx_price_prediction: lcxPricePredictionTool,
